...
type AppState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "success"; rows: CsvRow[]; address: string; total: number }
  | { status: "empty"; address: string }
    ...
    <div className="relative min-h-screen flex flex-col" style={{ background: "var(--bg-base)" }}>
      ...
      >
        ...
        <header className="text-center animate-fade-in">
          {/* Logo mark */}
          <div className="inline-flex items-center justify-center w-12 h-12 rounded-2xl mb-6 glow-orange"
        ...
        {state.status === "error" && (
          ...
          >
            ...
            <div>
              <p className="font-semibold text-sm" style={{ color: "#f87171", fontFamily: "var(--font-display)" }}>
        ...
        {state.status === "success" && (
          <>
            ...
            {/* Table preview */}
            <TransactionTable rows={state.rows} walletAddress={state.address} />
          </>
        )}
  ...
  const steps = [
    {
      ...
      desc: "Paste your SP or SM Stacks wallet address above.",
    },
    {
      icon: "⛓",
